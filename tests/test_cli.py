...
async def test_search_command(
    mock_args: argparse.Namespace, mock_vigicrues_client
) -> None:
...
@pytest.mark.asyncio
async def test_search_command_not_found(
    mock_args: argparse.Namespace, mock_vigicrues_client
) -> None:
...
@pytest.mark.asyncio
async def test_get_command_with_height_data(
    mock_args: argparse.Namespace, mock_vigicrues_client
) -> None:
    ...
    mock_vigicrues_client.get_latest_observations.assert_called_once_with(
        "O408101001", "H"
    )


@pytest.mark.asyncio
async def test_get_command_with_flow_data(
    mock_args: argparse.Namespace, mock_vigicrues_client
) -> None:
...
@pytest.mark.asyncio
async def test_get_command_with_both_data(
    mock_args: argparse.Namespace, mock_vigicrues_client
) -> None:
...
@pytest.mark.asyncio
async def test_territories_command(
    mock_args: argparse.Namespace, mock_vigicrues_client
) -> None:
...
@pytest.mark.asyncio
async def test_territories_command_no_results(
    mock_args: argparse.Namespace, mock_vigicrues_client
) -> None:
...
@pytest.mark.asyncio
async def test_troncons_command(
    mock_args: argparse.Namespace, mock_vigicrues_client
) -> None:
...
@pytest.mark.asyncio
async def test_troncons_command_no_results(
    mock_args: argparse.Namespace, mock_vigicrues_client
) -> None:
    """Test troncons CLI command."""
...
@pytest.mark.asyncio
async def test_stations_command(
    mock_args: argparse.Namespace, mock_vigicrues_client
) -> None:
...
@pytest.mark.asyncio
async def test_stations_command_no_results(
    mock_args: argparse.Namespace, mock_vigicrues_client
) -> None:
...
async def test_main_command_line_interface() -> None:
    ...
    with (
        patch(
            "vigicrues.cli.argparse.ArgumentParser.parse_args",
            return_value=argparse.Namespace(
                command="search", query="Sulpice", func=cli.search
            ),
        ),
    ...
    with (
        patch(
            "vigicrues.cli.argparse.ArgumentParser.parse_args",
            return_value=argparse.Namespace(
                command="get", station_id="O408101001", func=cli.get
            ),
        ),
        patch("vigicrues.cli.asyncio.run") as mock_run,
    ):
    ...
    with (
        patch(
            "vigicrues.cli.argparse.ArgumentParser.parse_args",
            return_value=argparse.Namespace(
                command="territories", func=cli.territories
            ),
        ),
        patch("vigicrues.cli.asyncio.run") as mock_run,
    ):
    ...
    with (
        patch(
            "vigicrues.cli.argparse.ArgumentParser.parse_args",
            return_value=argparse.Namespace(
                command="troncons", territory_id="25", func=cli.troncons
            ),
        ),
        patch("vigicrues.cli.asyncio.run") as mock_run,
    ):
    ...
    with (
        patch(
            "vigicrues.cli.argparse.ArgumentParser.parse_args",
            return_value=argparse.Namespace(
                command="stations", troncon_id="1", func=cli.stations
            ),
        ),
        patch("vigicrues.cli.asyncio.run") as mock_run,
    ):
